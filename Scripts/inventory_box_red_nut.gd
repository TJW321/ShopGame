...
func _process(_delta: float):
	if InventorySingleton.inventory_red_nut_count == 0:
		$TextureButton.texture_normal = inventory_count_empty
	elif InventorySingleton.inventory_red_nut_count >= 1:
		$TextureButton.texture_normal = inventory_count_full
	$Label.text = str(InventorySingleton.inventory_red_nut_count)

func _on_texture_button_pressed():
	if InventorySingleton.inventory_red_nut_count == 0:
		print("red nut box is empty chief")
	elif InventorySingleton.inventory_red_nut_count >= 1:
